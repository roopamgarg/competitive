 ...
 * @return {boolean}
 */
var isExist = function (board, word, i = 0, j = 0, indexes = {}) {
console.log(word)
  if (word.length === 0) {
    return true;
  }
  ...
  if (indexes[[i, j].toString()]) {
    return false;
  }
  if (board[i][j] === word[0]) {
    const newWord = word.slice(1);
    // indexes = {...indexes}
    indexes[[i, j].toString()] = true
    const down = isExist(board, newWord, i + 1, j, indexes); //down
    const right = isExist(board, newWord, i, j + 1, indexes); //right
    const up = isExist(board, newWord, i - 1, j, indexes); //up
    const left = isExist(board, newWord, i, j - 1, indexes); //left
    indexes[[i, j].toString()] = down || right || up || left;
...
const solve = (arr, word) => {
  console.log(word.length, arr.length * arr[0].length);
  for (let i = 0; i < arr.length; i++) {
    for (let j = 0; j < arr[i].length; j++) {
      if (isExist(arr, word, i, j)) {
        return true;
      }
    }
  }
